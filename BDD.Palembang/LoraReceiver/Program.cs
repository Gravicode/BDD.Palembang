...
namespace LoraReceiver
{
    public partial class Program
    {
        ...
        void ProgramStarted()
        {
            ...
            //set glide
            var xml = Resources.GetString(Resources.StringResources.Form1);
            ...
            //setup grid
            //create grid column
            GvData.AddColumn(new DataGridColumn("Time", 50));
            GvData.AddColumn(new DataGridColumn("Temp", 80));
            GvData.AddColumn(new DataGridColumn("Humid", 80));
            GvData.AddColumn(new DataGridColumn("Light", 80));
        ...
        void PrintToLCD(string message)
        {
            ...
            if (message != null && message.Length > 0)
            {
                try
                {
                    ...
                    //insert to db
                    var item = new DataGridItem(new object[] { TimeStr, data.temp, data.humid, data.light });
                    //add data to grid
                    GvData.AddItem(item);
                    Counter++;
